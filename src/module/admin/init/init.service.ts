...
export class InitService {
	...
	public async initRole() {
		const roles = [
			{ role_key: 'paker', role_name: '超级管理员', status: 1 },
			{ role_key: 'admin', role_name: '管理员', status: 1 },
	...
	public async initUser() {
		const users = [
			{ username: 'paker', password: '3633', nickname: '步沉香' },
			{ username: 'admin', password: '3633', nickname: '情雨随风' }
		]

		users.forEach(async params => {
			const auth = await this.userModel.findOne({ where: { username: params.username } })
			if (!auth) {
				const newUser = await this.userModel.create(params)
				const role = await this.roleModel.findOne({ where: { role_key: params.username } })
