...
export class UserService {
	...
	async createUser(user: createUserDto): Promise<User> {
		try {
			...
			const users = await (await this.userModel.findById(response.id, { password: 0, status: 0 })).toJSON()
			const auth = [
				{
					auth_name: '用户管理',
					apply: [
			...
			return await (
				await this.userModel.findById(response.id, { password: 0, status: 0 }).populate('roles')
			).toJSON()
		} catch (error) {
